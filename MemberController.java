...
public class MemberController {
	 ...
	 *  new 라는 키워드 없이 선언문만 써줘도 되지만 @Autowired 라는 어노테이션을 반드시 써줘야 함 
	 */
	...
	/*
	 * *스프링에서는 Controller를 일반 자바클래스로 만드는 대신 @Controller 어노테이션을 붙여서
	 * 컨트롤러임을 명시해준다.
	 * 
	 * *컨트롤러 클래스 내부에는 메소드를 만들어서 요청에 대한 처리내용을 작성한다.
	 * - 메소드는 무조건 public 으로 구현
	...
	public String loginMember(String userId, String userPwd) {
		...
		// 항상 경로명은 "/WEB-INF/views/member/~~.jsp"
		// 이 틀에 맞게 작성해야 하기 때문에
		// DispatcherServlet 의 설정파일인 servlet-context.xml 파일에서
		// view resolver 설정을 참조해서 응답페이지명을 지정해야 한다.
	...
	*/
	...
	/*
	 * * Spring 에서 요청 시 전달값을 뽑는 방법
	 * 
	 * 1. HttpServletRequest 객체를 이용하여 전달받기(기존의 jsp/servlet방식)
	 * 
	...
	@RequestMapping(value="login.me") //RequestMapping 어노테이션을 붙여줌으로써 컨트롤러로 등록
	                                  // login.me 요청이 들어왔을 때 DispatcherServlet 에 의해
	                                  // loginMember메소드가 호출됨!
	...
	*/
	 ...
	 *    만약, 넘어온 값이 비어있는 상태라면 defaultValue 속성으로 기본값을 지정해 줄 수 있다.
	 */
	...
	*/
	 ...
	 *  단, 매개변수명을 jsp 의 name 속성값과 동일하게 셋팅해둬야 자동으로 값이 주입됨
	 *  한가지 단점이라고 한다면 위에서 썻던 defaultValue같은 추가적인 속성을 사용할 수 없음
	 * 
	 */
	/*
	@RequestMapping(value="login.me")
	...
	*/
	 ...
	 * 4. 커맨드 객체 방식
	 *   해당 메소드의 매개변수로
	 *   요청 시 전달값을 담고자 하는 VO 클래스 타입을 셋팅 후
	 *   jsp 의 name 속성값을 VO 클래스에 담고자 하는 필드명으로 작성
	 *   
	 *   스프링 컨테이너가 해당 객체를 내부적으로 기본생성자를 이용해서 만들고
	 *   추가적으로 해당 값에 해당하는 setter 메소드들을 호출해서 가공해서 매개변수로 주입해줌
	 *   
	 *   반드시 name속성값과 해당 객체의 필드명이 일치해야함!(id, name 과 같게 해야 함)
	 * 
	 */
	/*
	 * *요청 처리 후 응답데이터를 담고 응답페이지로 포워딩 또는 url 재요청 하는 방법
	 * 
	 * 1.스프링에서 제공하는 Model객체(==requestScope 객체)를 사용하는 방법
	...
	public String loginMember(
		...
		//결과에 따른 응답뷰지정
		if(loginUser == null) { //로그인 실패=> 에러문구를 requestScope 에 담아 에러페이지 포워딩
			...
			// prefix + 내가 작성한주소 +suffix
			// "/WEB-INF/views/" + 내가작성한주소+".jsp"
			//return "WEB-INF/views/common/errorPage.jsp";
			return "common/errorPage"; 
			
		}
		else { //로그인 성공 =>loginUser 변수를 sessionScope 에 담고 메인페이지로 url 재요청
			
			//sessionScope 에 로그인한 회원의 정보 담기
			// 기존의 방식으로 session 객체 생성 :request.getSession();
			
			// 스프링에서는 내가 session을 이용하고자 할때
			// 메소드의 매개변수로 정의해두면 알아서 스프링 컨테이너에 의해서 객체 생성후 주입됨
			...
			return "redirect:/";  // /: contextPath 뒤에 붙는 /			
		}	
	}
	*/
	 ...
	 *   Model 은 데이터를 단순히 key - value 세트로 담는 역할이라고 한다면
	 *   View 는 응답뷰에 대한 정보를 담을 수 있는 역할
	 *   Model과 View 가 합쳐진 객체가 ModelAndView 객체임!
	 *   
	 *   이 경우에는 return 타입이 String 이 아닌 ModelAndView 타입이어야 한다.
	 *   추가적으로, Model 객체는 단독 존재가 가능(1번 방법으로 응답뷰를 지정 가능), View 는 단독 존재가 불가하다. 
	 *   
	 *   [표현법]
	 *   model 에 key-value 세트로 데이터를 담을 경우 : mv.addObject("키",밸류);
	 *   view 에 응답 뷰의 정보를 담을 경우 : mv.setViewName("포워딩할 jsp파일경로 또는 redirect:요청할 url");
	 */
	@RequestMapping(value="login.me")
	public ModelAndView loginMember(
		...
		// 일단 조회된 회원결과가 있는지 그리고 평문비밀번호와 암호문비밀버호가 일치하는지
		...
		if(loginUser == null) { //실패=> 에러문구를 requestScope 에 담아서 에러페이지로 포워딩
			...
			//ModelAndView 객체 사용할 경우
			mv.setViewName("common/errorPage"); //포워딩 방식
			
			//return mv;
		}
		else { //성공 => sessionScope 에 회원정보를 담고 메인페이지로 url 재요청
	...
	public String enrollForm() {
		...
		//View Resolver 설정에 의해 앞에는 prefix, 뒤에는 suffix가 붙임
	...
	public String insertMember(Member m, Model model, HttpSession session) {
		...
		// 2. 선택 입력사항을 적지 않고 요청을 보냈을 때 400에러(Bad Request Error) 발생
		//=> 어디서? age 필드는 int 형인데 만약 값을 입력하지 않은 경우 null값이 들어오기 때문
		//=> 해결방법 : Member 클래스의 age필드를 String 타입으로 변경
		
		//3. 비밀번호가 사용자가 입력한적 있는 그대로의 "평문(숨겨지지 않는 상태)"
		...
		//  => 보통은 bean 등록시 root-context.xml 파일이나 servlet-context.xml 파일에 
		//     등록하지만, 보통 암호화 관련된 bean 설정은 별도로 빼두는 경우가 많음
		//  =>  spring/spring-security.xml 파일 생성
		//  =>  bean 등록
		//  => web.xml 에 spring-security.xml 파일 읽히도록 설정
		//3) @Autowired을 이용하여 언제든지 bcryptPasswordEncoder 객체 사용가능하도록 설정
		...
		// m => userPwd 필드값을 encPwd으로 바꿔치기 후 insert 요청
		m.setUserPwd(encPwd);
	...
	public String updateMember(Member m, Model model, HttpSession session) {
		...
		if(result>0) { //성공
			...
			// session 에 loginUser 라는 키값으로 밸류를 덮어씌워줘야함 (로그인 상태가 계속 유지되게끔)
			...
			return "common/errorPage"; // 절대 경로 /WEB-INF/views/common/errorPage.jsp	
		}	
	}
	...
	public String deleteMember(String userPwd, String userId, HttpSession session,Model model) {
		...
		// 누구를 탈퇴시킬것인가? => 현재 로그인한 사용자의 아이디값
		// 1. <input type="hidden"> 로 넘겨오기
		// 2.  Controller단에서 session으로 알아내기
		...
		if(bCryptPasswordEncoder.matches(userPwd, encPwd)) { //비밀번호가 맞을경우 => 탈퇴처리
			
			int result = memberService.deleteMember(userId);
			
			if(result >0 ) { //탈퇴 성공 => session에서 loginUser 제거, alert문구 담기
				
				// invalidate메소드 쓰는 경우=> 이미 세션이 무효화되었기 때문에 추가적인 값을 담을수 없음
				//session.invalidate();
			...
			else { // 탈퇴 실패 => 에러문구를 담아서 에러페이지로 포워딩
		...
		else { //비밀번호가 맞지 않을 경우 => 메세지 + 마이페이지로 url 재요청
