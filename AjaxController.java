...
public class AjaxController {
	...
	public void ajaxMethod1(String name
		...
		// Servlet 때에는 doGet() 메소드 정의 부분에 throws구문이 자동완성 되어 있었지만
		// Spring 에서는 내가 직접 요청시 처리할  Controller 메소드를 만들어야 하기 때문에
		// 예외가 발생할법한 구문은 try~catch 블럭으로 직접 처리하거나 아니면
		// DispatcherServlet 이 예외를 처리할 수 있게끔 throws 구문을 붙인다. 
		
	}
	*/
	/*
	//2. 응답할 데이터를 문자열로 반환 ( HttpServletResponse객체를 안쓸 수 있음)
	@ResponseBody
	...
	public String ajaxMethod1(String name, int age) {
		...
		return responseData;
		// 404 오류 발생 : /WEB-INF/views/~~~~~.jsp로 응답페이지가 지정되었기 때문에 오류
		// => 만약에 응답 데이터로써 String 타입의 값을 리턴하고 싶다면
		//    내가 리턴할 값이 응답페이지와 관련된 내용이 아니라 응답데이터임을 명시해줘야 한다.
		//  @ResponseBody 어노테이션을 이용!
		//  단, 한글일 경우에는 @RequestMapping 어노테이션에 produces 속성으로 인코딩 설정도 해줘야 함!
		
	}
	*/
	// 응답데이터가 여러개일 경우=> 기존의 response 객체를 이용하는 방법
	/*
	@RequestMapping("ajax1.do")
	public void ajaxMethod1(String name
			               , int age
			               , HttpServletResponse response) throws IOException {
		//요청처리가 다 되었다는 가정 하에 여러개의 데이터 응답
		
		//response.setContentType("text/html; charset=UTF-8");
		//response.getWriter().print(name);
		//response.getWriter().print(age); //한개의 문자열로 연이여져서 돌아옴
	...
	*/
	...
	// 여러개의 응답데이터 보내기 =>Spring 방법 (response 객체 사용 안함)
	@ResponseBody
	...
	public String ajaxMethod3() {
		...
		//=> JDK 6버전이기 때문에 ArrayList<> 로 작성 불가능
		
		list.add(new Member("user01","pass01","홍길동",20,"01011112222"));
		list.add(new Member("user02","pass02","박지민",21,"01011113333"));
		list.add(new Member("user03","pass03","박효신",22,"01011114444"));
		list.add(new Member("user04","pass04","김남준",23,"01011115555"));
		list.add(new Member("user05","pass05","정세운",24,"01011116666"));
		list.add(new Member("user06","pass06","박서준",25,"01011117777"));
